...
def index():
    ...
    table_args = {"limit": 5, "offset": 0, "start_time": start, "end_time": end}
...
def invalid():
    ...
    return render_template("invalid.html", objects=objects)
...
def planner_metric(objid):
    ...
    return render_template("planner_metric.html", obj=obj, runs=runs)
...
def map_metrics():
    ...
    return render_template("maps.html", form=form, locations=locations)
...
def planner_trends():
    ...
    return render_template(
        "planner_trends.html", form=form, intervals=intervals, trend=trend
    )
...
def histograms():
    ...
    for i in range(1, len(intervals)):
        data.append(
            db.get_workflow_count_by_field(
                field, intervals[i - 1], intervals[i], start, end
            )
        )
    return render_template(
        "histograms.html", form=form, trend=data, intervals=intervals
    )
...
def download_trends():
    ...
    return render_template(
        "download_trends.html", form=form, intervals=intervals, trend=trend
    )
...
def to_multi_dict(d):
    ...
    if d is not None and not isinstance(d, MultiDict):
        return MultiDict([(k, v) for k, v in list(d.items())])
    else:
        return d
